  ...
  <div class="col-md-12">
    <div class="well well-sm">
      <ol class="breadcrumb" style="margin-bottom: -3px;">
        <li><strong>Select Salesman</strong></li>
       ...
       <div class="col-md-12">
        ...
        <div class="form-group">
         <form action="add_sales.php" method="post">
          ...
          <label for="salesman">List of Salesman</label>
          <select name="selectSalesman" class="form-control" required>
           <option value="">Select Salesman</option>
           <?php 
           $select = "SELECT * FROM users WHERE user_role = 'Salesman' AND branch = '$capitalPrefix'";
           $squery = mysqli_query($connection,$select);
           $count = 0;
           while($row = mysqli_fetch_array($squery)) {

             $id            = $row['user_id'];
             $salesmanFirst = $row['firstname'];
             $SalesmanLast  = $row['lastname'];
             $count++;
             ?>
             <option value='<?php echo $id; ?>'><?php echo ucwords($salesmanFirst)." ".ucwords($SalesmanLast); ?></option>";
             <?php } ?>
...
<?php if ($countT <= 0) { ?>
<script>
  bootbox.alert('Create a <a href="manage_drivers.php"><strong>Vehicle</strong></a> first!', function(){
