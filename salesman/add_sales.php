...
if (isset($_POST['addCart']) && $_POST['addCart']=="Add Items to Cart") {
 ...
 foreach($_POST['qtyBuy'] as $index=>$value){
   if ($value > 0) {
    ...
    (product_id, cartProdcode, cartProdname, cartProdesc, cartProdsell, cartProdexp, quantityCart, cartProdcat, cartProdempty) 
    VALUES ($cartProd_id, '$cartProd_code', '$cartProd_name', '$cartProd_desc', $cartProd_sellprice, '$cartProd_expdate', $value, $cartProdcat, $cartProdempty)
...
//REMOVE CASE FROM SHOPPING CART
        if (isset($_GET['removeCase'])) {
          $removeCaseID = $_GET['removeCase'];

          if (isset($cartQty)) {
            $updateQuery = "SELECT * FROM {$prefix}case_cart WHERE  case_cart_product_id = $removeCaseID;";
            $execSelectCart1 = mysqli_query($connection, $updateQuery);

            while ($row = mysqli_fetch_array($execSelectCart1)) {
              $qCart = $row['case_quantityCart'];
            }
            if (isset($qCart)) {

              $updateQuery .= "UPDATE {$prefix}case SET case_quantity = case_quantity + $qCart WHERE  case_product_id = $removeCaseID;";  
              $updateQuery .= "DELETE FROM {$prefix}case_cart WHERE case_cart_product_id  = $removeCaseID;";   
              mysqli_multi_query($connection, $updateQuery);
              header("Refresh:0; add_sales.php");
...
//remove all    
        if (isset($_GET['removeall'])) {
          ...
          $updateQuery = "SELECT * FROM {$prefix}case_cart";
          $execSelectCart1 = mysqli_query($connection, $updateQuery);

          while ($row = mysqli_fetch_array($execSelectCart1)) {
            $qCartID = $row['case_cart_product_id'];
            $qCart = $row['case_quantityCart'];

            $updateQuery = "UPDATE {$prefix}case SET case_quantity = case_quantity + $qCart WHERE case_product_id = $qCartID"; 
